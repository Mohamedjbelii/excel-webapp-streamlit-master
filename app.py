...
if authentication_status :
    ...
    #------------Load data
        ...
        #------------create_table_of_activities
            for i in w:
                fig = go.Figure(data=[go.Table(header=dict(values=['year','Acitivity','MM/WF CODE','Duration ','Average']),
                                 cells=dict(values=[dct.get(i)['Year&Week'],dct.get(i)['Activity'],dct.get(i)['MM/WF CODE'],dct.get(i)['Duration'],dct.get(i)['Average'] ],
                                           line_color='darkslategray',
                                           fill_color='lightcyan',
                                           align='left'))])
                fig.update_layout(
                ...
                st.write(fig)
                st.dataframe(dct.get(i).drop(['Year','Week'], axis=1))
        #------------create_figures_of_every_product_in_each_activities

            for i in w :

                fig = px.bar(x= dct.get(i)['Year&Week'],
                    y= dct.get(i)['Duration'], color=dct.get(i)["MM/WF CODE"], labels={"color": "product name", "x": "time spent","y":"Week"})
                fig.update_layout(
                title=f"{i} figure",
                yaxis_title="Hours",
                xaxis_title="Date",)
